...
public class Dico extends DefaultHandler {
    ...
    /*
        ...
        ________________________________________________________________________________________________
     */
    public void lireDictionnaireDom(String path, String filname) {
        try {
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder p = dbFactory.newDocumentBuilder();
            Document doc = p.parse(path +"/"+filname);
            //on obtient une liste de mot
            NodeList t = doc.getElementsByTagName("mot");
            //on parcours la liste puis on recupere pour chaque mot sont niveaau (getAttribute )et le mot (getTextContent)
            //on peut pas appliquer un for -each sur une liste de noeud
            for (int i = 0; i <= t.getLength(); i++) {
                Element mot = (Element) t.item(i);
                String mots=mot.getTextContent();
                int niveau = Integer.parseInt(mot.getAttribute("niveau"));

                //methode deja implementer plus haut
                ajouteMotADico(niveau, mots);
            }
